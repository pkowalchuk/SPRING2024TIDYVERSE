...
##Additional code by Andrianina David Raharijao:

Hello, I will be adding some code to make some visualization using ggplot2

```{r package_load}
library(ggplot2)
```

Bar chart to visualize the number of flights reviewed per airline

```{r bar1}
ggplot(flightReviewData, aes(x = Airline)) +
  geom_bar() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(title = "Number of Flight Reviews per Airline",
       x = "Airline",
       y = "Count of Reviews")
```

Box plot looking at the distribution of overall ratings by class.

```{r bar2}
ggplot(flightReviewData, aes(x = Class, y = Overall.Rating)) +
  geom_boxplot() +
  labs(title = "Distribution of Overall Ratings by Class",
       x = "Class",
       y = "Overall Rating")
```
