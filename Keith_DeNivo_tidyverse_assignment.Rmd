...
Data set uploaded to the tidyverse git. Data is read into a dataframe.
...
drops unused levels. Below, we drop the test drive data and the
corresponding level from the factor categorizing the number of owners of
a car.
...
The `o2` factor was created with 4 levels as displayed above.
...
fct_infreq can be used to sort by the frequency of the factor. The
numerical values for the y values of factors can be reordered using
fct_reorder fct_reorder() and fact_infreq were used to reorder data
frames and graphs.
